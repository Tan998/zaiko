...
if (is_submit('delete_remove_item'))
{
    // Lấy ID và ép kiểu
    $id = input_post('id');
    if ($id)
    {
        ...
        $delete_xuatkho_ct = db_get_row(db_create_sql('SELECT * FROM xuatkho_ct {where}', array(
            'STT' => $id
        )));
            $sql2 = db_create_sql('DELETE FROM xuatkho_ct {where}', array(
                'STT' => $id
            ));
            if (db_execute($sql2)){
                ?>
