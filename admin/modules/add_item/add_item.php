		...
		<div class="card text-center px-2 py-2">
			...
			<div class="card-body">
				...
				<div>
					...
					<div class="col-md-6 row" id="scanner_box">
...
<div class="container card my-2 mb-5">
	<form id="myform" class="my-3" action="" method="POST">
		...
		<input class="btn border border-info" type="submit" name="add_item_submit" value="次へ">
	</form>
<?php 
if (isset($_POST['add_item_submit'])) {
	...
	if ($bin_code_val != "") {
	    ...
	    if (count($item_data) > 0) { 
	    	...
	    	?>
	    	<form method="POST" action="<?php echo create_link(base_url('admin/index.php'), array('m' => 'add_item', 'a' => 'add_item_check')); ?>" >
	    		<label for="add_item_value" class="form-label mb-2">入庫数量を入力</label>
	    		<div class="col-12">
	    			<span class="col-2 bg-light bg-gradient border rounded-pill text-center p-1">在庫数: 　<u><strong><?php get_zaikosuu($bin_code_val); ?></strong></u></span>
		    		<span class="col-1">+</span>
		    		<input hidden type="text" name="bin_code_val" value="<?php echo $bin_code_val ?>">
		    		<input class="col-6" type="number" min="0" max="999" required name="add_item_number" placeholder="入庫数を入力してください。" value="" class="form-control" id="add_item_value">
	    		</div>
				<input class=" my-2 btn btn-success" type="submit" name="add_item_check" value="入庫">
			</form>
<?php 	}
	    else{
	    	echo "<script>
