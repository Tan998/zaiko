	...
	<div class="px-5 py-2">
			...
			<form id="main-form" method="post" onsubmit="return confirm('この情報で入庫しますか？');" action="<?php echo create_link(base_url('admin/index.php'), array('m' => 'add_item', 'a' => 'add_item_check')); ?>">
				...
				<div class="mb-3">
					...
					<textarea class="form-control" id="memo" placeholder="入庫メモを入力" name="memo" value=""><?php print( input_post('memo')); ?></textarea>
				...
				<div class="row">
					...
					<div class="col-12 col-sm-6 mb-3">
						<label for="add_item_number" class="form-label bg-light bg-gradient rounded-pill text-center px-1"><strong>入庫数</strong></label>
						<span class="badge rounded-pill bg-danger" style="font-size: 10px;">必須</span>

						<input disabled class="form-control" id="add_item_number" type="number" min="1" max="999" required value="<?php print(input_post('add_item_number')); ?>">
						<input hidden type="text" name="add_item_number" value="<?php print(input_post('add_item_number')); ?>">
						<input hidden type="text" name="MaHang" value="<?php print(input_post('bin_code_val')); ?>">
						<input hidden type="text" name="bin_code_val" value="<?php print(input_post('bin_code_val')); ?>">
						<?php //show_error($error, 'add_item_number'); ?>
					</div>
				...
				<div class="mb-3">
					<a class="btn btn-success" onclick="$('#main-form').submit()" href="#">入庫完了</a>
