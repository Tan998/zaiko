<?php if (!defined('IN_SITE')) die ('The request not found');
    ...
    $sql = "SELECT hanghoa.id, hanghoa.bin_code,nhapkho_ct.STT,hanghoa.MaHang,hanghoa.DVT, hanghoa.TenHang, nhapkho_ct.SLNhap, nhapkho.NgayNhap,nhapkho_ct.update_time, nhapkho_ct.NguoiPhuTrach
    ...
    $sql = "SELECT hanghoa.id, hanghoa.bin_code,nhapkho_ct.STT,hanghoa.MaHang,hanghoa.DVT, hanghoa.TenHang, nhapkho_ct.SLNhap, nhapkho.NgayNhap,nhapkho_ct.update_time, nhapkho_ct.NguoiPhuTrach
                            FROM hanghoa INNER JOIN (nhapkho INNER JOIN nhapkho_ct ON nhapkho.SoPhieuN = nhapkho_ct.SoPhieuN) ON hanghoa.MaHang = nhapkho_ct.MaHang
                            WHERE nhapkho.NgayNhap<= ".date("Ymd")." AND nhapkho_ct.acc_action = '".get_current_username()."' order by nhapkho.NgayNhap desc,nhapkho_ct.STT desc";
    $users = db_get_list($sql);
    }
?>
<!doctype html>
<html lang="en">
    ...
    <body id="nyuko_ichiran_lazy">
            <div class="col-12 row bg-white">
                <div class="my-3">
                    <h4>入庫一覧</h4>
                    <div class="border-bottom border-3"></div>
                </div>
                <table class="my_table_all table table-hover table-borderless row" id="my_table1">
                    <thead class="col-12">
                                        ...
                                        <div class="col-sm-12 text-start">
                                          <div class="h-100 row">
                                          	<div class="col-12 text-white border-bottom bg-secondary bg-gradient d-flex justify-content-between">
                                                <span class="my-1">入庫ID: <?php echo $item['STT']; ?></span>
                                          	...
                                          	<div class="status_div px-0">
                                        		...
                                        		<div class="card-text row my-1">
	                                                <small class="col-6 border-end px-0 row">
	                                                    ...
	                                                    <span class="item_name"><strong>商品番号: <?php echo $item['bin_code']; ?></strong></span><br>
	                                                    <span>データ更新日: <?php echo $item['update_time']; ?></span>
	                                                </small>
	                                                <div class="col-6 card-text text-end d-flex flex-column">
                                                        <span>入庫日: <?php echo $item['NgayNhap']; ?></span><br>
                                                        <span><strong>入庫数: <?php echo $item['SLNhap']; ?></strong></span>
                                                    </div>
                                            	</div>
                                        	</div>
                                          </div>
